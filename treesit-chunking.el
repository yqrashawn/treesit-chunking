;;; treesit-chunking.el --- treesit based chunking utilities -*- lexical-binding: t; -*-
;;
...
(defgroup treesit-chunking nil
  "Tree-sitter based code splitter for RAG applications."
  :group 'tools
  :prefix "treesit-chunking-")

(defcustom treesit-chunking-chunk-size 2500
...
(defvar treesit-chunking--symbol-patterns
...
(defvar treesit-chunking--enhanced-dependency-patterns
...
(defcustom treesit-chunking-min-chunk-size 100
...
(defvar treesit-chunking-language-map
  '((js . javascript)
    (ts . typescript)
    (py . python)
    (c++ . cpp)
...
(defun treesit-chunking--extract-dependencies (content language)
...
(defun treesit-chunking--extract-enhanced-dependencies (content language)
  ...
  (when treesit-chunking-track-dependencies
    (let ((patterns (alist-get language treesit-chunking--enhanced-dependency-patterns))
          (dependencies '()))
      (when patterns
        (dolist (pattern-pair patterns)
...
(defun treesit-chunking--enhance-chunk-with-lsp (chunk pos &optional original-buffer)
  "Enhance CHUNK with LSP metadata at POS using ORIGINAL-BUFFER for LSP context."
  (let* ((content (treesit-chunking-chunk-content chunk))
         (node-type (treesit-chunking-chunk-node-type chunk))
         (language (treesit-chunking-chunk-language chunk))
         ;; Extract basic information
         (def-name (treesit-chunking--extract-function-name content node-type))
         (symbols-defined (treesit-chunking--extract-symbols-defined content language))
         (symbols-used (treesit-chunking--extract-enhanced-symbols-used content language))
         (complexity (treesit-chunking--calculate-complexity content))
         (checksum (secure-hash 'md5 content))
         (dependencies (treesit-chunking--extract-enhanced-dependencies content language))
         ;; Get LSP information from original buffer
         (hover-info (treesit-chunking--get-lsp-hover-info pos original-buffer))
         (symbol-info (treesit-chunking--get-lsp-symbol-info pos original-buffer)))

    ;; Set enhanced metadata
    (setf (treesit-chunking-chunk-definition-name chunk) def-name)
    (setf (treesit-chunking-chunk-definition-type chunk)
          (treesit-chunking--normalize-definition-type node-type))
    (setf (treesit-chunking-chunk-hover-info chunk) hover-info)
    (setf (treesit-chunking-chunk-symbols-defined chunk) symbols-defined)
    (setf (treesit-chunking-chunk-symbols-used chunk) symbols-used)
    (setf (treesit-chunking-chunk-complexity-score chunk) complexity)
    (setf (treesit-chunking-chunk-checksum chunk) checksum)
    (setf (treesit-chunking-chunk-dependencies chunk) dependencies)

    ;; Extract signature and docstring from LSP if available
    (when symbol-info
      (setf (treesit-chunking-chunk-signature chunk)
            (lsp:symbol-information-name symbol-info)))

    ;; Extract docstring from content
    (setf (treesit-chunking-chunk-docstring chunk)
          (treesit-chunking--extract-docstring content language))))


(defun treesit-chunking--normalize-language (language)
  "Normalize LANGUAGE name to canonical form."
  (let ((lang-symbol (if (stringp language)
                         (intern language)
                       language)))
    (or (alist-get lang-symbol treesit-chunking-language-map)
        lang-symbol)))

(defun treesit-chunking--get-splittable-types (language)
  "Get splittable node types for LANGUAGE."
  (let ((normalized-lang (treesit-chunking--normalize-language language)))
    (alist-get normalized-lang treesit-chunking-splittable-node-types)))

(defun treesit-chunking--language-supported-p (language)
  "Check if LANGUAGE is supported by tree-sitter splitter."
  (let ((normalized-lang (treesit-chunking--normalize-language language)))
    (and (assoc normalized-lang treesit-chunking-splittable-node-types)
         (treesit-language-available-p normalized-lang))))

(defun treesit-chunking--node-text (node)
  "Get text content of NODE."
  (buffer-substring-no-properties
   (treesit-node-start node)
   (treesit-node-end node)))

(defun treesit-chunking--node-line-range (node)
...
(defun treesit-chunking--clojure-definition-p (content)
...
(defun treesit-chunking--extract-clojure-definition-type (content node-type language)
...
(defun treesit-chunking--extract-ast-chunks (root-node splittable-types language file-path &optional original-buffer)
      ...
      (treesit-search-subtree
       ...
       (lambda (node)
         (when (memq (intern (treesit-node-type node)) splittable-types)
           (let* ((start-pos (treesit-node-start node))
                  (end-pos (treesit-node-end node))
                  (line-range (treesit-chunking--node-line-range node))
                  (start-line (car line-range))
                  (end-line (cdr line-range))
                  (content (treesit-chunking--node-text node))
             ...
             (unless (range-overlaps-p start-line end-line)
               ...
               (when (and (eq language 'clojure) (string= node-type "list_lit"))
                 ...
                 (unless (treesit-chunking--clojure-definition-p content)
                   (setq content nil))) ; Skip this node

               (when (and content (> (length (string-trim content)) treesit-chunking-min-chunk-size))
                 (let ((chunk (make-treesit-chunking-chunk
                               ...
                               :language language
                               :file-path file-path
                               :node-type (treesit-chunking--extract-clojure-definition-type content node-type language))))
                   ;; Enhance with LSP metadata if available
                   (treesit-chunking--enhance-chunk-with-lsp chunk start-pos original-buffer)
                   (push chunk chunks)
...
(defun treesit-chunking--split-large-chunk (chunk)
  ...
  (let* ((content (treesit-chunking-chunk-content chunk))
    ...
    (if (member node-type '("class_declaration" "class_specifier" "namespace_definition" "mod_item"))
        (treesit-chunking--split-large-chunk-semantically chunk)
      ;; For other large chunks, use simple line-based splitting but mark as fragments
      (treesit-chunking--split-large-chunk-by-lines chunk))))

(defun treesit-chunking--split-large-chunk-semantically (chunk)
             ...
             (lambda (node)
               (when (memq (intern (treesit-node-type node)) internal-types)
                 (let* ((start-pos (treesit-node-start node))
                   ...
                   (when (> (length (string-trim node-content)) treesit-chunking-min-chunk-size)
                     (push (make-treesit-chunking-chunk
                            :content node-content
                            :start-line (+ (treesit-chunking-chunk-start-line chunk) line-start -1)
                            :end-line (+ (treesit-chunking-chunk-start-line chunk) line-end -1)
                            :language language
                            :file-path (treesit-chunking-chunk-file-path chunk)
                            :node-type (treesit-node-type node))
                           found-internals))))
               nil)
             t)
...
(defun treesit-chunking--split-large-chunk-by-lines (chunk)
  "Split a large CHUNK by lines, preserving metadata for the first sub-chunk."
  (let* ((content (treesit-chunking-chunk-content chunk))
         (lines (split-string content "\n" t))
         (current-chunk "")
         (current-start-line (treesit-chunking-chunk-start-line chunk))
    ...
    (dolist (line lines)
      (let ((line-with-newline (concat line "\n")))
        (if (and (> (length current-chunk) 0)
            ...
            ;; Create a sub-chunk
            (progn
              (let ((sub-chunk (make-treesit-chunking-chunk
                                ...
                                :node-type (if first-chunk-p
                                               (treesit-chunking-chunk-node-type chunk)
                                             (concat (treesit-chunking-chunk-node-type chunk) "-fragment")))))
                ;; Preserve metadata for the first sub-chunk
                (when first-chunk-p
                  (setf (treesit-chunking-chunk-definition-name sub-chunk)
                        (treesit-chunking-chunk-definition-name chunk))
                  (setf (treesit-chunking-chunk-definition-type sub-chunk)
                        (treesit-chunking-chunk-definition-type chunk))
                  (setf (treesit-chunking-chunk-signature sub-chunk)
                        (treesit-chunking-chunk-signature chunk))
                  (setf (treesit-chunking-chunk-docstring sub-chunk)
                        (treesit-chunking-chunk-docstring chunk))
                  (setf (treesit-chunking-chunk-hover-info sub-chunk)
                        (treesit-chunking-chunk-hover-info chunk))
                  (setf (treesit-chunking-chunk-symbols-used sub-chunk)
                        (treesit-chunking-chunk-symbols-used chunk))
                  (setf (treesit-chunking-chunk-complexity-score sub-chunk)
                        (treesit-chunking-chunk-complexity-score chunk))
...
(defcustom treesit-chunking-fallback-strategies
...
(defcustom treesit-chunking-context-lines 2
  "Number of lines of context to include around chunks for better understanding."
  :type 'integer
  :group 'treesit-chunking)

(defvar treesit-chunking--language-boundaries
...
(defun treesit-chunking--try-fallback-strategies (content language file-path &optional original-buffer)
  "Try multiple fallback strategies for splitting CONTENT.
ORIGINAL-BUFFER is the source buffer for context."
  (let ((strategies treesit-chunking-fallback-strategies)
        chunks)
    ...
    (while (and strategies (null chunks))
      (let ((strategy (car strategies)))
        ...
        (setq chunks
              (condition-case err
                  (pcase strategy
                    ('imenu (treesit-chunking--imenu-split content language file-path original-buffer))
                    ('intelligent-line (treesit-chunking--intelligent-line-split content language file-path))
                    ('simple-line (treesit-chunking--simple-line-split content language file-path)))
                (error
    ...
    (or chunks
        ...
        (progn
          ...
          (treesit-chunking--simple-line-split content language file-path)))))

(defun treesit-chunking--imenu-split (content language file-path &optional original-buffer)
      ...
      (when (and (boundp 'imenu-create-index-function)
        ...
        (let* ((imenu-index (condition-case nil
          ...
          (when imenu-index
            ...
            (setq chunks (treesit-chunking--process-imenu-entries
                          imenu-index content language file-path))
            ;; Add context and enhance chunks
            (when chunks
              (setq chunks (treesit-chunking--add-context-to-chunks chunks content))
              (setq chunks (treesit-chunking--enhance-fallback-chunks chunks language)))
            chunks))))))
        ...
        (when (and is-boundary
          ...
          (let ((chunk-content (mapconcat 'identity current-chunk-lines "\n")))
            (push (make-treesit-chunking-chunk
                   ...
                   :definition-name (treesit-chunking--extract-definition-from-line line language)
...
(defun treesit-chunking--get-language-boundaries (language)
  "Get boundary patterns for LANGUAGE."
  (or (alist-get language treesit-chunking--language-boundaries)
      ;; Generic fallback patterns
      '("^\\s-*function" "^\\s-*class" "^\\s-*def" "^\\s-*//" "^\\s-*#")))

(defun treesit-chunking--line-matches-boundaries (line boundaries)
  "Check if LINE matches any of the BOUNDARIES patterns."
  (cl-some (lambda (pattern)
             (string-match-p pattern line))
           boundaries))

(defun treesit-chunking--extract-definition-from-line (line language)
    ...
    (let ((lines (split-string content "\n"))
      ...
      (mapcar (lambda (chunk)
                (let* ((start-line (treesit-chunking-chunk-start-line chunk))
                       ...
                       (context-start (max 1 (- start-line treesit-chunking-context-lines)))
...
(defun treesit-chunking--enhance-fallback-chunks (chunks language)
  "Enhance fallback CHUNKS with metadata for LANGUAGE."
  (mapcar (lambda (chunk)
            (let* ((content (treesit-chunking-chunk-content chunk))
                   (complexity (treesit-chunking--calculate-complexity content))
                   (symbols-used (treesit-chunking--extract-symbols-used content language))
                   (checksum (secure-hash 'md5 content)))
              (setf (treesit-chunking-chunk-complexity-score chunk) complexity)
              (setf (treesit-chunking-chunk-symbols-used chunk) symbols-used)
              (setf (treesit-chunking-chunk-checksum chunk) checksum)
...
(defun treesit-chunking--add-overlap (chunks)
  "Add overlap between CHUNKS."
  (if (or (<= (length chunks) 1)
          (<= treesit-chunking-chunk-overlap 0))
      chunks
    (let ((overlapped-chunks '()))
      (dotimes (i (length chunks))
        (let* ((chunk (nth i chunks))
               (content (treesit-chunking-chunk-content chunk))
               (metadata (copy-treesit-chunking-chunk chunk)))

          ;; Add overlap from previous chunk
          (when (> i 0)
            (let* ((prev-chunk (nth (1- i) chunks))
                   (prev-content (treesit-chunking-chunk-content prev-chunk))
                   (overlap-text (substring prev-content
                                            (max 0 (- (length prev-content)
                                                      treesit-chunking-chunk-overlap)))))
              (setf (treesit-chunking-chunk-content metadata)
                    (concat overlap-text "\n" content))))

          (push metadata overlapped-chunks)))
      (nreverse overlapped-chunks))))

(defun treesit-chunking--fallback-split (content language file-path)
...
;;;###autoload
(defun treesit-chunking-split-buffer (&optional language file-path)
...
FILE-PATH is the file path for metadata (uses buffer-file-name if nil)."
  ...
  (let* ((language (or language
                       (treesit-chunking--detect-language)))
         (file-path (or file-path
                        (buffer-file-name)
                        (buffer-name)))
         (content (buffer-string))
         (original-buffer (current-buffer)))
    (treesit-chunking-split-string content language file-path original-buffer)))

;;;###autoload
(defun treesit-chunking-split-string (content language &optional file-path original-buffer)
  "Split CONTENT string into chunks using tree-sitter.
LANGUAGE is the programming language.
FILE-PATH is optional file path for metadata.
ORIGINAL-BUFFER is the source buffer for LSP context."
  (let ((normalized-lang (treesit-chunking--normalize-language language)))
    (if (not (treesit-chunking--language-supported-p normalized-lang))
        (progn
          (message "Language %s not supported by tree-sitter splitter, using fallback strategies" language)
          (treesit-chunking--try-fallback-strategies content normalized-lang file-path original-buffer))
      ...
      (with-temp-buffer
        ...
        (condition-case err
            (let* ((parser (treesit-parser-create normalized-lang))
                   (root-node (treesit-parser-root-node parser))
                   (splittable-types (treesit-chunking--get-splittable-types normalized-lang)))
              ...
              (let ((chunks (treesit-chunking--extract-ast-chunks
                ...
                (let ((refined-chunks '()))
                  (dolist (chunk chunks)
                    (if (<= (length (treesit-chunking-chunk-content chunk))
                            treesit-chunking-chunk-size)
                        (push chunk refined-chunks)
                      (setq refined-chunks
                            (append (treesit-chunking--split-large-chunk chunk)
                                    refined-chunks))))

                  (treesit-chunking--add-overlap (nreverse refined-chunks)))))
...
(defun treesit-chunking--lsp-available-p (&optional buffer)
...
(defun treesit-chunking--get-lsp-hover-info (pos &optional buffer)
  "Get LSP hover information at POS in BUFFER (or current buffer)."
  (when (treesit-chunking--lsp-available-p buffer)
    (with-current-buffer (or buffer (current-buffer))
      (condition-case nil
          (save-excursion
            (goto-char pos)
...
(defun treesit-chunking--get-lsp-symbol-info (pos &optional buffer)
      ...
      (condition-case nil
          (save-excursion
            ...
            (let* ((symbols (lsp-request "textDocument/documentSymbol"
              ...
              (treesit-chunking--find-symbol-at-position symbols target-line target-col)))
        (error nil)))))

(defun treesit-chunking--find-symbol-at-position (symbols line col)
    ...
    (dolist (symbol symbols)
      (let* ((range (lsp:symbol-information-location symbol))
        ...
        (when-let ((children (lsp:document-symbol-children symbol)))
          (let ((found (treesit-chunking--find-symbol-at-position children line col)))
            (when found (throw 'found found))))))))

(defun treesit-chunking--extract-function-name (content node-type)
...
(defun treesit-chunking--extract-symbols-used (content language)
...
(defun treesit-chunking--count-matches (regexp string)
...
(defun treesit-chunking--calculate-complexity (content)
  "Calculate rough complexity score for CONTENT."
  (let ((complexity 1))
    ;; Count control structures
    (setq complexity (+ complexity
                        (* 2 (treesit-chunking--count-matches "\\(if\\|when\\|cond\\|case\\|switch\\)" content))
                        (* 3 (treesit-chunking--count-matches "\\(for\\|while\\|loop\\|doseq\\)" content))
                        (* 1 (treesit-chunking--count-matches "\\(and\\|or\\|&&\\|||\\)" content))))
    ;; Factor in length
    (setq complexity (+ complexity (/ (length content) 100)))
    complexity))

(defun treesit-chunking--normalize-definition-type (node-type)
...
(defun treesit-chunking--extract-docstring (content language)
...
(defun treesit-chunking--detect-language ()
...
;;;###autoload
(defun treesit-chunking-split-file (file-path &optional language)
...
LANGUAGE is optional and will be auto-detected if nil."
  (with-temp-buffer
    ...
    (let ((detected-lang (or language
                             (treesit-chunking--detect-language))))
      (treesit-chunking-split-string (buffer-string) detected-lang file-path))))

(defun treesit-chunking-chunk-to-plist (chunk)
  "Convert CHUNK to a plist representation."
  (list :content (treesit-chunking-chunk-content chunk)
        :start-line (treesit-chunking-chunk-start-line chunk)
        :end-line (treesit-chunking-chunk-end-line chunk)
        :language (treesit-chunking-chunk-language chunk)
        :file-path (treesit-chunking-chunk-file-path chunk)
        :node-type (treesit-chunking-chunk-node-type chunk)
        ;; Enhanced metadata
        :definition-name (treesit-chunking-chunk-definition-name chunk)
        :definition-type (treesit-chunking-chunk-definition-type chunk)
        :signature (treesit-chunking-chunk-signature chunk)
        :docstring (treesit-chunking-chunk-docstring chunk)
        :hover-info (treesit-chunking-chunk-hover-info chunk)
        :symbols-defined (treesit-chunking-chunk-symbols-defined chunk)
        :symbols-used (treesit-chunking-chunk-symbols-used chunk)
        :dependencies (treesit-chunking-chunk-dependencies chunk)
        :complexity-score (treesit-chunking-chunk-complexity-score chunk)
        :checksum (treesit-chunking-chunk-checksum chunk)
        :related-chunks (treesit-chunking-chunk-related-chunks chunk)))

(defun treesit-chunking-chunks-to-json (chunks)
  "Convert CHUNKS to JSON representation."
  (require 'json)
  (json-encode (mapcar #'treesit-chunking-chunk-to-plist chunks)))

;;;###autoload
(defun treesit-chunking-demo ()
  "Demo function to test the splitter on current buffer."
  (interactive)
  (let* ((original-buffer (current-buffer))
         (chunks (treesit-chunking-split-buffer))
         (buf (get-buffer-create "*treesit-chunking-demo*")))
    (with-current-buffer buf
      (erase-buffer)
      (insert (format "Found %d chunks (LSP: %s):\n\n"
                      (length chunks)
                      (with-current-buffer original-buffer
                        (let ((lsp-result (treesit-chunking--lsp-available-p)))
      ...
      (dotimes (i (length chunks))
        (let ((chunk (nth i chunks)))
          ...
          (insert (format "Lines: %d-%d\n"
                          (treesit-chunking-chunk-start-line chunk)
                          (treesit-chunking-chunk-end-line chunk)))
          (insert (format "Language: %s\n" (treesit-chunking-chunk-language chunk)))
          (insert (format "Node Type: %s\n" (treesit-chunking-chunk-node-type chunk)))
          (insert (format "Size: %d chars\n" (length (treesit-chunking-chunk-content chunk))))

          ;; Enhanced metadata display
          (when-let ((def-name (treesit-chunking-chunk-definition-name chunk)))
            (insert (format "Definition: %s (%s)\n"
                            def-name
                            (or (treesit-chunking-chunk-definition-type chunk) "unknown"))))

          (when-let ((signature (treesit-chunking-chunk-signature chunk)))
            (insert (format "Signature: %s\n" signature)))

          (when-let ((docstring (treesit-chunking-chunk-docstring chunk)))
            (insert (format "Docstring: %s\n" (substring docstring 0 (min 100 (length docstring))))))

          (when-let ((symbols (treesit-chunking-chunk-symbols-used chunk)))
            (insert (format "Symbols Used: %s\n" (mapconcat 'identity (take 5 symbols) ", "))))

          (when-let ((complexity (treesit-chunking-chunk-complexity-score chunk)))
            (insert (format "Complexity: %d\n" complexity)))

          (when-let ((hover-info (treesit-chunking-chunk-hover-info chunk)))
            (insert (format "LSP Info: %s\n" (substring hover-info 0 (min 150 (length hover-info))))))

          (insert "Content:\n")
          (insert (treesit-chunking-chunk-content chunk))
          (insert "\n\n"))))
    (pop-to-buffer buf)))
...
;;;###autoload
(defun treesit-chunking-analyze-file (file-path)
  ...
  (let* ((existing-buffer (find-buffer-visiting file-path))
         ...
         chunks
         (buf (get-buffer-create "*treesit-chunking-analysis*")))
    ;; Get chunks from the source buffer (which has LSP if available)
    (with-current-buffer source-buffer
      (setq chunks (treesit-chunking-split-buffer)))
    ...
    (with-current-buffer buf
      ...
      (insert (format "LSP Available: %s\n\n"
                      (with-current-buffer source-buffer
                        (if (treesit-chunking--lsp-available-p) "Yes" "No"))))
      ...
      (let ((total-complexity 0)
        ...
        (dolist (chunk chunks)
          ...
          (setq avg-size (+ avg-size (length (treesit-chunking-chunk-content chunk))))
          (when (equal (treesit-chunking-chunk-definition-type chunk) "function")
            (setq function-count (1+ function-count)))
          (when (equal (treesit-chunking-chunk-definition-type chunk) "class")
      ...
      (dotimes (i (length chunks))
        (let ((chunk (nth i chunks)))
          (insert (format "Chunk %d: %s\n"
                          (1+ i)
                          (or (treesit-chunking-chunk-definition-name chunk) "unnamed")))
          (insert (format "  Type: %s, Lines: %d-%d, Complexity: %d\n"
                          (or (treesit-chunking-chunk-definition-type chunk) "unknown")
                          (treesit-chunking-chunk-start-line chunk)
                          (treesit-chunking-chunk-end-line chunk)
                          (or (treesit-chunking-chunk-complexity-score chunk) 0)))
          (when-let ((symbols (treesit-chunking-chunk-symbols-used chunk)))
